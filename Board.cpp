...
void Board::EvalMove(pair<int, int> from, pair<int, int> to) {
...
void Board::InputAndMakeNextMove() {
	...
	string indata1;
	string indata1ToLower;
	int fromX, fromY, toX, toY;


	// From
	cout << "Next move... (or 'exit')" << endl << "From: ";
	cin >> indata1 >> fromY;

	// ToLower
	for (auto ch : indata1) {
		if (ch != ' ') {
			indata1ToLower.push_back(ch);
		}
	}

	// Check if exit or other
	if (indata1 == "exit") {
		Board::isExit = true;
		return;
	}

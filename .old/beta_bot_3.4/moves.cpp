...
// Generate Moves
void GenerateMoves(const U64 kBB[13], U64 moves[100], const bool kSide, bool a) {
	...
	if (kSide) {
		...
		pieceBitboard = kBB[6];
		while (pieceBitboard) WKingMoves(PopLsb(pieceBitboard), kWBlock, kBBlock, moves, kBB, a);
	}
...
inline void WKingMoves(const int kSquare, const U64 kWBlock, const U64 kBBlock, U64 moves[100], const U64 kBB[13], bool a) {
	...
	if (w_short_castle && !(kWCastleShortSpaceMask & (kWBlock | kBBlock)) && !(kBB[7] & kWCastleShortPawnMask) && !(kBB[8] & kWCastleShortKnightMask) && !(kBB[12] & kWCastleShortKingMask)) {
		...
		for (int square = 60; square < 63; ++square) {
			...
			for (int x = t_rank - 1, y = t_file - 1; x > -1 && y > -1; --x, --y) {
				...
				if (1ULL << (x * 8 + y) & kWBlock) break;
				else if (1ULL << (x * 8 + y) & (kBB[9] | kBB[11])) { can_castle = false; if (a) cout << "1"; }
			...
			for (int x = t_rank - 1; x > -1; --x) {
				...
				else if (1ULL << (x * 8 + t_file) & (kBB[10] | kBB[11])) { can_castle = false; if (a) cout << "2"; }
			...
			for (int x = t_rank - 1, y = t_file + 1; x > -1 && y < 8; --x, ++y) {
				...
				if (1ULL << (x * 8 + y) & kWBlock) break;
				else if (1ULL << (x * 8 + y) & (kBB[9] | kBB[11])) { can_castle = false; if (a) cout << "3"; }
				else if (1ULL << (x * 8 + y) & kBBlock) break;
			}
		}
